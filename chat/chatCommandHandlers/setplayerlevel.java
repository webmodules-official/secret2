...
public class setplayerlevel implements ChatCommandExecutor {
	 private Map<String, Character> names = new HashMap<String, Character>();
	
	public void execute(String[] parameters, Connection source) {
		//System.out.println("Handling change character level command");
		for(int i=0;i<parameters.length;i++) {
			//System.out.println("Command param[" + (i+1) + "] : " + parameters[i]);
				...
				anyplayer.setLevel(Integer.parseInt(parameters[1]));
				CharacterDAO.changeplayerlevel(Integer.parseInt(parameters[1]), anyplayer.getLOGsetName());
				anyplayer.sendToMap(anyplayer.extCharPacket());	
						//TODO: resond party chat that to player that level has been changed
