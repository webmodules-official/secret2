...
public class porttele implements ChatCommandExecutor {
	public  int inc = 0;
	...
	public void execute(String[] parameters, Connection source) {
		...
		float gox = Integer.valueOf(parameters[0]);
		float goy = Integer.valueOf(parameters[1]);
		int gomap = Integer.valueOf(parameters[2]);
		if(gomap == 9 && cur.getLevel() < 130){Charstuff.getInstance().respondguild("You need to be at least Level 130 in order to enter Sky Zone.", cur.GetChannel()); return;}
		cur.setX(gox);
		cur.setY(goy);
		...
		cur.partyUID = 0; // remove char from any party
		cur.getitemdata();
		...
		byte[] cha = new byte[] {
			...
			// standard shit 
			byte[] xCoord0 = BitTools.floatToByteArray(gox);
			byte[] yCoord0 = BitTools.floatToByteArray(goy);
			for(int i=0;i<4;i++) {
