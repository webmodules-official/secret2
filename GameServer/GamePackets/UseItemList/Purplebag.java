...
public class Purplebag implements UseItemCommandExecutor {
	

	public boolean execute(int ItemID, int DETERMINER, Connection source) {
		Character cur = ((PlayerConnection)source).getActiveCharacter();
		
		int limit = 12;// Size of every
						...
						if(inc == 5){
							...
							if(inc == 6){
								...
								if(inc == 7){
									...
									if(inc == 8){
										source.addWrite(Item.itemSpawnPacket(cur.getCharID(),  283000209, 1, cur.getlastknownX(), cur.getlastknownY(), source));	
									}else 
										if(inc == 9){
											source.addWrite(Item.itemSpawnPacket(cur.getCharID(),  283000022, 1, cur.getlastknownX(), cur.getlastknownY(), source));	
									}else
										if(inc == 10){
											source.addWrite(Item.itemSpawnPacket(cur.getCharID(),  273000080, 1, cur.getlastknownX(), cur.getlastknownY(), source));		
									}
									else if(inc == 11){
										source.addWrite(Item.itemSpawnPacket(cur.getCharID(),  283000106, 1, cur.getlastknownX(), cur.getlastknownY(), source));		
									}
									else if(inc == 12){
