...
import GameServer.GamePackets.Login;
...
public class ServerCore implements Runnable{
	...
	private int readQsize = 200; //default read/write queue size for each
	private int writeQsize = 800; //new connection. can be adjusted while running(applies to new connections)
	
	private ConnectionCollector con; 
	private PacketHandler phand;
	private InetSocketAddress bindi;
	
	private final int selectorCount = 10;
	...
	private void init(InetSocketAddress bind, ConnectionCollector con) {		
		try {
			...
			this.con = con;
			if (this.sSocket.getLocalPort() == 11000){}else{
		...
		for(int i=0;i<this.selectorCount;i++) {
			SelectorThread st;
			try {
				st = new SelectorThread(Selector.open(), 5000, 5000);			
