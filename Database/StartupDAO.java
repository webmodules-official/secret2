...
public class StartupDAO {
	...
	public static void loadMaps(){
		...
		Grid grid;
			try{
				ResultSet rs = SQLconnection.getInstance().executeQuery(Queries.getMaps(SQLconnection.getInstance().getaConnection()));
				while( rs.next()){
	...
	public static void setZones(){
		...
		ServerLogger log = ServerLogger.getInstance();
		try{
			ResultSet rs = Queries.getZones(SQLconnection.getInstance().getConnection()).executeQuery();
			while(rs.next()){
				map = rs.getInt("map");
				x = rs.getInt("startx");
				y = rs.getInt("starty");
